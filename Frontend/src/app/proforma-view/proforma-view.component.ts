...
export class ProformaViewComponent implements OnInit {
  ...
  public displayedColumns1 = ['slno', 'prepared_by', 'type', 'description', 'part_name', 'hsn', 'qty',
    'uom', 'total', 'submitted_date', 'print_without_logo', 'icici_bank_print',
    'sbi_bank_print', 'both_bank_print', 'print_with_signature', 'print_oaitem', 'edit', 'delete'];
  ...
  materialDate: any = '';
  freight: any = 0;
  getBasicValuePercentage: any = 0; //new added
  getGSTPercentange: any = 0; // new added
  totalBasicValue: any = 0; // new added
  totalGSTValue: any = 0; // new 
  ...
  getIs_special_user() {
    this.is_special_user = sessionStorage.getItem("is_special");
    if (this.is_special_user == 'false') {
      this.displayedColumns1 = this.displayedColumns1.filter(column => column !== 'print_with_signature');
    }
  }
  ...
  proformaView() {
    ...
    this.apiService.getData(url + this.proformaId).then((result: any) => {
      ...
      if (result.status != false) {
        ...
        console.log("this.dataElement", this.dataElement);
  ...
  getBasicValuePercentageFunc(event: any) {
    const getBasicValuePercentage = parseFloat(event.target.value);
    if (getBasicValuePercentage != 0) {
      const totalUnitPrice = this.dataItems.map((element: any) => this.totalValueWithGST).reduce((total, value) => value == null ? null : parseFloat(total) + parseFloat(value), 0);
      this.totalBasicValue = (totalUnitPrice * getBasicValuePercentage / 100).toFixed(2);
    }
    this.calculate('total_amt');
  }

  getGSTPercentangeFunc(event: any) {
    // if getBasicValuePercentageFunc is already called then this.totalValue will be updated
    if (this.getBasicValuePercentage != 0) {
      this.totalValue = this.totalBasicValue;
    }
    else {
      this.totalValue = this.dataItems.map((element: any) => this.totalValueWithGST).reduce((total, value) => value == null ? null : parseFloat(total) + parseFloat(value), 0);
    }
    const getGSTPercentange = parseFloat(event.target.value);
    this.totalGSTValue = (this.totalValue * getGSTPercentange / 100).toFixed(2);
    console.log('totalGSTValue', this.totalGSTValue)
    this.calculate('total_amt')
  ...
  calculate(val) {
    ...
    if (this.dataItems != undefined) {
      ...
      if (val == 'unit_price') {
        ...
        if (this.getBasicValuePercentage != 0) {
          data = this.totalBasicValue;
        }
        else if (this.getBasicValuePercentage != 0 && this.getGSTPercentange != 0) {
          data = this.totalGSTValue
        }
        else {
  ...
  // Based on PI Qty calculation


  generatePDF(element, index, type) {

    if (type == "print_without_logo") {
    ...
    else if (type == "sbi_bank_print") {
      let url = "get_order_ack/print_sbi_bank_with_logo/" + this.orderAcknowledgementData[index].OrderAckId;
      this.apiService.downloadPDF(url).then((data) => {
        var downloadURL = window.URL.createObjectURL(data);
        let tab = window.open();
        tab.location.href = downloadURL;
    ...
    else if (type == "both_bank_print") {
      let url = "get_order_ack/invoice_with_icici_sbi/" + this.orderAcknowledgementData[index].OrderAckId;
      console.log(url);
      this.apiService.downloadPDF(url).then((data) => {
        var downloadURL = window.URL.createObjectURL(data);
        let tab = window.open();
    ...
    else if (type == "print_with_signature") {
      let url = "get_order_ack/print_with_signature/" + this.orderAcknowledgementData[index].OrderAckId;
      console.log(url);
      this.apiService.downloadPDF(url).then((data) => {
        var downloadURL = window.URL.createObjectURL(data);
        let tab = window.open();
    ...
    // ao item
      ...
      this.apiService.downloadPDF(url).then((data) => {
        var downloadURL = window.URL.createObjectURL(data);// var link = document.createElement('a');
        let tab = window.open();
        tab.location.href = downloadURL;
      });
