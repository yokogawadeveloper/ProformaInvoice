...
export class ProformaViewComponent implements OnInit {
  ...
  materialDate: any = '';
  freight: any = 0;
  getBasicValuePercentage: any = 0; //new by gautam 05-02-2024
  getGSTPercentange: any = 0; //new by gautam 05-02-2024
  ...
  getTotalValue(): number {
    ...
    // Assuming you have the necessary properties for calculation
    let totalPriceWithGST: number = 0;
    let totalUnitPrice: number = 0;
    let totalFreight: number = 0;
    let totalGST: number = 0;
    let addTCSAmount: number = 0;

    // Your logic for calculating total value based on lineItemType 'M'
    if (this.lineItemType == 'M') {
      ...
      // Calculate total value based on your logic
      totalValue = totalPriceWithGST + totalUnitPrice + totalFreight + totalGST + addTCSAmount;
    }
    return totalValue;
  }
  ...
  GstBasicValuePercentage(fieldName: string, event: any) {
    const basicValuePercentage = parseFloat(event.target.value);
    console.log(basicValuePercentage);
    const totalValue = this.getTotalValue();
    const basicValue = (basicValuePercentage * totalValue) / 100;
    // Update the basic value property
    this.getBasicValuePercentage = basicValue;
    this.onCalculation();
  }
