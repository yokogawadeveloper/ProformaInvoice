...
class orderAcknowledgementViewSet(viewsets.ModelViewSet):
    ...
    def amount_to_words(amount):
        ...
        dollar_words = orderAcknowledgementViewSet.convert_number_to_words(dollars)
        cent_words = orderAcknowledgementViewSet.convert_cents_to_words(cents)
    ...
    def list(self, request, *args, **kwargs):
        ...
        return Response(
            {'proforma': proforma_serializer_data, 'records': serializer_data, 'totalRecords': total_records})
    ...
    def create(self, request, *args, **kwargs):
        ...
        if queryset.filter(ProformaID=request.data['ProformaID']).exists():
            count = queryset.filter(ProformaID=request.data['ProformaID']).values('ProformaID').annotate(count=Count('ProformaID'))
            data['RevNo'] += count[0]['count']
    ...
    def update(self, request, *args, **kwargs):
        ...
        data['PI_CODE'] = "P" + '-' + division_serializer.data['Abbr'] + '-' + region_serializer.data[
            'Abbr'] + '-' + year[-2:] + '-' + pi_code_suffix[-1]
    ...
    def delete_item(self, request):
        data = request.data
        orderAcknowledgement.objects.filter(OrderAckId=data['order_ack_id']).update(DeleteFlag=True,
                                                                                    DeletedBy_id=request.user.id,
                                                                                    deleted_remarks=data['remarks'])
        orderAcknowledgementHistory.objects.filter(OrderAckId_id=data['order_ack_id']).update(DeleteFlag=True,
                                                                                              DeletedBy_id=request.user.id)
    ...
    def print_with_logo(self, request, id):
        pi_instance = orderAcknowledgement.objects.get(pk=id)
        serializer = self.serializer_class(pi_instance, context={'request': request})
        data = serializer.data
    ...
    def print_without_logo(self, request, id):
        pi_instance = orderAcknowledgement.objects.get(pk=id)
        serializer = self.serializer_class(pi_instance, context={'request': request})
        data = serializer.data
        filename = "Invoice__%s.pdf" % pi_instance.OrderAckId
    ...
    def proforma_report_list(self, request):
        data = request.data
    ...
    # By Gautam Ankul
    @action(methods=['get'], detail=False, url_path='print_sbi_bank_with_logo/(?P<id>[0-9]+)')
    def print_sbi_bank_with_logo(self, request, id):
        pi_instance = orderAcknowledgement.objects.get(pk=id)
        serializer = self.serializer_class(pi_instance, context={'request': request})
        data = serializer.data
        filename = "Invoice__%s.pdf" % pi_instance.OrderAckId
        ...
        pdf_response = PDFTemplateResponse(
            request=request,
            template='print_sbi_bank_with_logo.html',
            filename=filename,
            context=data,
            cmd_options={
                'title': filename,
    ...
    @action(methods=['get'], detail=False, url_path='invoice_with_icici_sbi/(?P<id>[0-9]+)')
    def invoice_with_icici_sbi(self, request, id):
        pi_instance = orderAcknowledgement.objects.get(pk=id)
        serializer = self.serializer_class(pi_instance, context={'request': request})
        data = serializer.data
        filename = "Invoice__%s.pdf" % pi_instance.OrderAckId
    ...
    @action(methods=['get'], detail=False, url_path='print_with_signature/(?P<id>[0-9]+)')
    def print_with_signature(self, request, id):
        pi_instance = orderAcknowledgement.objects.get(pk=id)
        serializer = self.serializer_class(pi_instance, context={'request': request})
        data = serializer.data
        filename = "Invoice__%s.pdf" % pi_instance.OrderAckId
        ...
        current_user_signature = get_object_or_404(UserSignature, user=current_user)
        signature_path = current_user_signature.signature.path
        with open(signature_path, "rb") as image_file:
            signature_data = image_file.read()
        encoded_signature = base64.b64encode(signature_data).decode('ascii')

        # Create a data URL for the image
        image64 = f'data:image/png;base64,{encoded_signature}'
        data['yil_signature'] = image64

        # For only header logo
        path = os.path.abspath('static/images/header-image.png/')
        with open(path, "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode('ascii')
            image64 = 'data:image/png;base64,{}'.format(encoded_string)
        ...
        pdf_response = PDFTemplateResponse(
            request=request,
            template='invoice_with_signature.html',
            filename=filename,
            context=data,
            cmd_options={
                'title': filename,
    ...
    @action(methods=['get'], detail=False, url_path='invoice_for_gdci_mizuho_bank/(?P<id>[0-9]+)')
    def invoice_for_gdci_mizuho_bank(self, request, id):
        pi_instance = orderAcknowledgement.objects.get(pk=id)
        serializer = self.serializer_class(pi_instance, context={'request': request})
        data = serializer.data
        filename = "Invoice__%s.pdf" % pi_instance.OrderAckId
        ...
        data['total_amount_with_tcs_in_words'] = self.amount_to_words(amount=data['TotalAmountWithTCS'])
        # return Response(data)
        pdf_response = PDFTemplateResponse(
            request=request,
            template='invoice_for_gdci_mizuho_bank.html',
            filename=filename,
            context=data,
            cmd_options={
                'title': filename,
    ...
    @action(methods=['get'], detail=False, url_path='invoice_for_gdci_mufg_bank/(?P<id>[0-9]+)')
    def invoice_for_gdci_mufg_bank(self, request, id):
        pi_instance = orderAcknowledgement.objects.get(pk=id)
        serializer = self.serializer_class(pi_instance, context={'request': request})
        data = serializer.data
        filename = "Invoice__%s.pdf" % pi_instance.OrderAckId
        ...
        pdf_response = PDFTemplateResponse(
            request=request,
            template='invoice_for_gdci_mufg_bank.html',
            filename=filename,
            context=data,
            cmd_options={
                'title': filename,
